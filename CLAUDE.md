...
- **Backend**: Node.js (`main.js` and `lib/`). Python has been eliminated.
- **FFI**: `koffi` is used for native Windows API calls (e.g. `NtSuspendProcess` for process suspension).
...
2. **Process Management**: Windows process suspension is handled cleanly via `koffi` and `ntdll.dll`'s `NtSuspendProcess`.
