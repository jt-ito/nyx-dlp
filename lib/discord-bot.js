...
class DiscordBotManager {
  ...
  discordRequest(endpoint, method = 'GET', data = null, headers = {}) {
    return new Promise((resolve, reject) => {
      ...
      const pkgVersion = require('../package.json').version || '3.0.11';
      const reqHeaders = {
  ...
  async start(opts = {}) {
    ...
    this.stop(); // Clean up any active connection
  ...
  handleGatewayPayload(payload) {
    ...
    switch (op) {
      case 10: { // Hello: Start Heartbeat & Identify
        ...
        this.sendGateway(2, {
          token: this.token,
          intents: 0, // Slash commands do not require privileged gateway intents!
          properties: {
            os: os.platform(),
            browser: 'nyx-dlp',
            device: 'nyx-dlp'
      ...
      case 9: // Invalid Session
        this.stop();
        setTimeout(() => this.connectGateway(), 2000);
        break;
    }
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      const req = https.request({
        ...
        headers: {
          ...
          'User-Agent': `DiscordBot (nyx-dlp, ${require('../package.json').version || '3.0.11'})`
        },
        timeout: 60000
      }, (res) => {
        res.on('data', () => {});
  ...
  async executeCommand(commandName, options, interaction) {
    ...
    const destDir = this.downloadDir || path.join(os.homedir(), 'Downloads', 'nyx-dlp');
    ...
    if (commandName === 'status') {
      ...
      await this.updateInteractionResponse(token, {
        ...
        fields: [
          ...
          { name: 'Engine Version', value: `nyx-dlp v${require('../package.json').version || '3.0.11'}`, inline: true }
        ],
    ...
    const broadcast = async (data) => {
      if (data.type === 'stdout' || data.type === 'stderr') {
        ...
        // Extract percentage, speed, ETA if present
        const match = text.match(/(\d+(\.\d+)?%)\s+of\s+~?[\d.]+[A-Za-z]+\s+at\s+([\d.]+[A-Za-z]+\/s)\s+ETA\s+([\d:]+)/i);
        if (match) {
          job.lastPercent = match[1];
          job.lastSpeed = match[3];
          job.lastEta = match[4];
        }
      ...
      if (data.type === 'exit') {
        ...
        // Check if output files were produced and under 25MB to attach
        const filesToAttach = [];
        try {
          if (exitCode === 0 && fs.existsSync(destDir)) {
            const dirFiles = fs.readdirSync(destDir).map(f => path.join(destDir, f));
            // Find files modified in the last 2 minutes
            const recentFiles = dirFiles.filter(f => {
              try {
                const stat = fs.statSync(f);
                return stat.isFile() && (Date.now() - stat.mtimeMs < 120000);
