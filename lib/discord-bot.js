...
class DiscordBotManager {
  ...
  getEffectiveDownloadDir() {
    ...
    if (!dir || dir === 'undefined' || dir === 'null') {
      try {
        const settingsStore = require('./settings-store.js');
        const sDir = settingsStore.getSettingValue('discordDownloadDir') || settingsStore.getSettingValue('discord-download-dir') || '';
        if (sDir && sDir !== 'undefined' && sDir !== 'null') {
          dir = sDir;
        }
      } catch (_) {}
    }
  ...
  async start(opts = {}) {
    ...
    if (opts.downloadDir && opts.downloadDir !== 'undefined' && opts.downloadDir !== 'null') {
      this.downloadDir = String(opts.downloadDir).trim();
    }
  ...
  connectGateway() {
    ...
    const gatewayUrl = (this.sessionId && this.resumeGatewayUrl)
      ? `${this.resumeGatewayUrl}${this.resumeGatewayUrl.includes('?') ? '&' : '/?'}v=10&encoding=json`
      : DISCORD_GATEWAY;

    // Set connection/handshake watchdog timeout (15 seconds)
    this.handshakeTimeout = setTimeout(() => {
      console.warn('[Discord Bot] Gateway connection handshake timed out (15s). Terminating and retrying...');
      if (this.ws) {
    ...
    this.ws.on('close', (code, reason) => {
      const reasonStr = (Buffer.isBuffer(reason) ? reason.toString('utf8') : (reason || '')).trim() || 'No reason / abnormal closure';
      console.log(`[Discord Bot] Gateway closed (${code}): ${reasonStr}`);
      this.cleanupHeartbeat();

      // Handle Discord-specific fatal / non-recoverable close codes
      if (code === 4004) {
        console.error('[Discord Bot] Authentication failed: Invalid bot token.');
        this.notifyStatus('error', { error: 'Authentication failed (invalid bot token)' });
        this.isManualStop = true;
        return;
      }
      if (code === 4010 || code === 4011 || code === 4012 || code === 4013 || code === 4014) {
        console.error(`[Discord Bot] Fatal Gateway error (${code}): ${reasonStr}`);
        this.notifyStatus('error', { error: `Fatal Gateway error (${code}): ${reasonStr}` });
        this.isManualStop = true;
        return;
      }

      // Handle session expiry or invalid sequence: discard session to force clean Identify
      if (code === 4007 || code === 4009 || this.reconnectAttempts >= 3) {
        this.sessionId = null;
        this.sequence = null;
        this.resumeGatewayUrl = null;
      }
    ...
    this.ws.on('error', (err) => {
      ...
      // Let 'close' or handshake watchdog handle reconnection
    });
  }
  ...
  resolveDownloadOptions(url, userOptions = {}) {
    const settingsStore = require('./settings-store.js');
    const { DEFAULT_SITE_PRESETS, DEFAULT_DISCORD_DOWNLOAD_OPTIONS, matchSitePreset } = require('./site-presets.js');

    // 1. Load saved site presets
    let presets = DEFAULT_SITE_PRESETS;
    try {
      const savedPresetsRaw = settingsStore.getSettingValue('site-presets');
      if (savedPresetsRaw) {
        const parsed = typeof savedPresetsRaw === 'string' ? JSON.parse(savedPresetsRaw) : savedPresetsRaw;
        if (Array.isArray(parsed) && parsed.length > 0) presets = parsed;
      }
    } catch (_) {}

    // 2. Load saved discord defaults
    const defFormat = settingsStore.getSettingValue('discord-def-format', DEFAULT_DISCORD_DOWNLOAD_OPTIONS.format);
    const defContainer = settingsStore.getSettingValue('discord-def-container', DEFAULT_DISCORD_DOWNLOAD_OPTIONS.container);
    const defConcurrent = parseInt(settingsStore.getSettingValue('discord-def-concurrent', DEFAULT_DISCORD_DOWNLOAD_OPTIONS.concurrent)) || 5;
    const defSubs = settingsStore.getSettingValue('discord-def-subs', DEFAULT_DISCORD_DOWNLOAD_OPTIONS.dlSubs);
    const defThumb = settingsStore.getSettingValue('discord-def-thumb', DEFAULT_DISCORD_DOWNLOAD_OPTIONS.dlThumb);
    const defDesc = settingsStore.getSettingValue('discord-def-desc', DEFAULT_DISCORD_DOWNLOAD_OPTIONS.dlDesc);
    const defAutoRepair = settingsStore.getSettingValue('discord-def-autorepair', DEFAULT_DISCORD_DOWNLOAD_OPTIONS.autoRepair);
    const defCookies = settingsStore.getSettingValue('discord-def-cookies', DEFAULT_DISCORD_DOWNLOAD_OPTIONS.useCookies);

    // 3. Match site preset
    const matched = matchSitePreset(url, presets);

    const format = userOptions.format || (matched && matched.format) || defFormat;
    const container = userOptions.container || (matched && matched.container) || defContainer;
    const client = userOptions.client || (matched && matched.client) || 'default';
    const concurrent = (matched && matched.concurrent) ? matched.concurrent : defConcurrent;
    const dlSubs = (matched && matched.dlSubs !== undefined) ? matched.dlSubs : !!defSubs;
    const embedSubs = (matched && matched.embedSubs !== undefined) ? matched.embedSubs : !!defSubs;
    const dlThumb = (matched && matched.dlThumb !== undefined) ? matched.dlThumb : !!defThumb;
    const embedThumb = (matched && matched.embedThumb !== undefined) ? matched.embedThumb : !!defThumb;
    const dlDesc = (matched && matched.dlDesc !== undefined) ? matched.dlDesc : !!defDesc;
    const dlComments = (matched && matched.dlComments !== undefined) ? matched.dlComments : false;
    const dlChat = (matched && matched.dlChat !== undefined) ? matched.dlChat : false;
    const autoRepair = (matched && matched.autoRepair !== undefined) ? matched.autoRepair : !!defAutoRepair;
    const useCookies = (matched && matched.useCookies !== undefined) ? matched.useCookies : !!defCookies;

    // Resolve cookies path if enabled
    let cookiesPath = '';
    if (useCookies) {
      const savedCookies = settingsStore.getSettingValue('yd-cookies') || settingsStore.getSettingValue('field:yd-cookies') || '';
      if (savedCookies && typeof savedCookies === 'string' && fs.existsSync(savedCookies)) {
        cookiesPath = savedCookies;
      }
    }

    let extraArgs = [];
    if (matched && matched.extraArgs) {
      extraArgs.push(...matched.extraArgs.split(/\s+/).filter(Boolean));
    }
    if (userOptions.extra_args) {
      try {
        const parsed = JSON.parse(userOptions.extra_args);
        extraArgs.push(...(Array.isArray(parsed) ? parsed : [userOptions.extra_args]));
      } catch (_) {
        extraArgs.push(...userOptions.extra_args.split(/\s+/).filter(Boolean));
      }
    }
...
function makeAsciiProgressBar(percentStr, barLength = 10) {
  ...
  return '`[' + '█'.repeat(filled) + '░'.repeat(empty) + ']` ' + `**${num.toFixed(1)}%**`;
    ...
    try {
      switch (commandName) {
        case 'ytdlp': {
          const resolved = this.resolveDownloadOptions(options.url, options);
          runners.runYtdlp({
            outputDir: destDir,
            url: options.url,
            ...
            cookiesPath: resolved.cookiesPath,
            dlSubs: resolved.dlSubs,
            embedSubs: resolved.embedSubs,
            dlThumb: resolved.dlThumb,
            embedThumb: resolved.embedThumb,
            dlDesc: resolved.dlDesc,
            dlComments: resolved.dlComments,
            dlChat: resolved.dlChat,
            autoRepair: resolved.autoRepair,
        ...
        case 'batch': {
          ...
          const resolved = this.resolveDownloadOptions(firstUrl, options);
          runners.runBatch({
            outputDir: destDir,
            urls: urlList,
            format: resolved.format,
