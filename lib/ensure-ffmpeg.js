...
async function ensureFfmpeg(requestedVersion = 'auto', sendLog = console.log) {
  ...
  try {
    ...
    if (archiveExt === '.zip') {
      ...
      // Flatten: find ffmpeg.exe and move it up
      const files = execSync(`dir /s /b "${VENDOR_DIR}\\ffmpeg.exe"`, { encoding: 'utf-8' }).trim().split('\n');
      if (files.length > 0 && files[0]) {
        const foundFfmpeg = files[0].trim();
        const binDir = path.dirname(foundFfmpeg);
        fs.copyFileSync(foundFfmpeg, path.join(VENDOR_DIR, 'ffmpeg.exe'));
        
        const foundFfprobe = path.join(binDir, 'ffprobe.exe');
        if (fs.existsSync(foundFfprobe)) {
          fs.copyFileSync(foundFfprobe, path.join(VENDOR_DIR, 'ffprobe.exe'));
        }
      }
    } else {
