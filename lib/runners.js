  ...
  const sweepStaleTempFolders = async (dir) => {
    try {
      ...
      for (const item of items) {
        if (item.startsWith('.temp_hls_') || item.startsWith('temp_hls_')) {
...
async function runYtdlp(opts, broadcast) {
  let subOnlyMeta = opts._subOnlyMeta || null;
  ...
  if (isClip) {
    ...
    const endTag = opts.endTime ? opts.endTime.replace(/:/g, '.') : 'end';
    if (subOnlyMeta) {
      args.push('-o', `${subOnlyMeta.title} [${subOnlyMeta.vodId}] [${startTag}-${endTag}].%(ext)s`);
    } else {
      args.push('-o', `%(title)s [%(id)s] [${startTag}-${endTag}].%(ext)s`);
    }
  } else if (subOnlyMeta) {
    args.push('-o', `${subOnlyMeta.title} [${subOnlyMeta.vodId}].%(ext)s`);
  }

  if (subOnlyMeta) {
    if (subOnlyMeta.title) {
      args.push('--parse-metadata', `${subOnlyMeta.title.replace(/:/g, '\\:')}:%(title)s`);
    }
    const vodUrl = subOnlyMeta.vodId ? `https://www.twitch.tv/videos/${subOnlyMeta.vodId}` : (opts._originalTwitchUrl || opts.url);
    if (vodUrl) {
  ...
  if (opts.autoRepair || opts.startTime || opts.endTime || ((opts.twitchSubOnly === true || opts.twitchSubOnly === 'true') && isTwitchVodUrl(opts.url))) childOpts.suppressExit = true;
...
async function downloadDirectM3u8Native(url, opts, ffmpeg, metadataArgs, output, broadcast) {
  ...
  broadcast({ type: 'stdout', text: `▶ [Native HLS] Downloading ${downloadTasks.length} fragments at native speed (15x concurrency)...\n` });
  ...
  async function fetchChunk(chunkUrl, dest, retries = 5) {
    ...
    for (let i = 0; i < retries; i++) {
      ...
      try {
        await new Promise((resolve, reject) => {
          const req = client.get(chunkUrl, { headers: { 'User-Agent': 'Mozilla/5.0' }, timeout: 15000 }, (res) => {
...
async function downloadSingleM3u8(url, opts, ffmpeg, broadcast) {
  ...
  if (isAutoTitle) {
    try {
      const meta = await resolveTwitchVodMetadata({
        url,
