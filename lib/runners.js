...
async function runGalleryDl(opts, broadcast) {
  ...
  if (!fs.existsSync(gdlBin)) {
    broadcast({ type: 'stdout', text: `[setup] gallery-dl standalone binary not found. Please install gallery-dl to vendor/gallery-dl/${exe}\n` });
    broadcast({ type: 'exit', code: 1 });
    return;
  }
  ...
  for (const f of opts.files) {
    ...
    try {
      if (fs.statSync(f).isDirectory()) {
        base += path.sep; // Append trailing slash to prevent folder name duplication bug in IA CLI
      }
    } catch(e) {}
