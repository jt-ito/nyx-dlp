...
const { ensureTreespend } = require('./ensure-treespend');
...
function spawnRunner(exe, args, opts, broadcast) {
  ...
  let maxDownloadedFrag = 0;
  let maxKnownFrag = 0;
  let hasMerged = false;

  const updateFragFromText = (text) => {
    if (!opts || !opts.isLivestream) return;
    const dMatches = text.matchAll(/\(frag\s+(\d+)(?:\/(\d+))?\)/gi);
    for (const m of dMatches) {
      if (m[1]) maxDownloadedFrag = Math.max(maxDownloadedFrag, parseInt(m[1], 10));
      if (m[2]) maxKnownFrag = Math.max(maxKnownFrag, parseInt(m[2], 10));
    }
    const skipMatches = text.matchAll(/(?:Skipping fragment|fragment not found;?\s*Skipping fragment)\s+(\d+)/gi);
    for (const m of skipMatches) {
      if (m[1]) maxKnownFrag = Math.max(maxKnownFrag, parseInt(m[1], 10));
    }
    const dashMatch = text.match(/\[dashsegments\]\s+Total fragments:\s+(\d+)/i);
    if (dashMatch) {
      maxKnownFrag = Math.max(maxKnownFrag, parseInt(dashMatch[1], 10));
    }
    const liveEdgeMatch = text.match(/\[live\]\s+Detected live broadcast edge:\s*~?(\d[\d,]*)/i);
    if (liveEdgeMatch) {
      maxKnownFrag = Math.max(maxKnownFrag, parseInt(liveEdgeMatch[1].replace(/,/g, ''), 10));
    }
    if (text.includes('[Merger] Merging formats into') || text.includes('Deleting original file')) {
      hasMerged = true;
    }
  };
  ...
  child.stdout.on('data', (data) => {
    ...
    updateFragFromText(txt);
    broadcast({ type: 'stdout', text: txt });
  });
  child.stderr.on('data', (data) => {
    const txt = data.toString();
    updateFragFromText(txt);
    broadcast({ type: 'stderr', text: txt });
  });

  child.on('close', async (code) => {
    activeProcs.delete(child.pid);
    if (!child._isKilledByUser && opts.tempDir && !opts.keepTemp && !opts.suppressExit) {
      const isLiveStream = !!opts.isLivestream && (maxKnownFrag > 0 || maxDownloadedFrag > 0);
      const isCompleteLive = isLiveStream && (
        maxDownloadedFrag >= maxKnownFrag ||
        (maxKnownFrag - maxDownloadedFrag <= 3 && hasMerged)
      );

      if (isLiveStream) {
        if (isCompleteLive) {
          if (fs.existsSync(opts.tempDir) && fs.readdirSync(opts.tempDir).some(f => !f.endsWith('.part') && !f.endsWith('.ytdl'))) {
            await moveFilesUp(opts.tempDir, opts.outputDir);
          }
        } else {
          broadcast({
            type: 'stdout',
            text: `\n📁 Incomplete files retained in: ${opts.tempDir}\n`
          });
        }
      } else {
        if (code === 0 || (fs.existsSync(opts.tempDir) && fs.readdirSync(opts.tempDir).some(f => !f.endsWith('.part') && !f.endsWith('.ytdl')))) {
          await moveFilesUp(opts.tempDir, opts.outputDir);
        }
      }
    }
...
function detectLiveStreamEdge(url) {
  return new Promise((resolve) => {
    try {
      if (!url || (!url.includes('youtube.com') && !url.includes('youtu.be'))) return resolve(null);
      const req = https.get(url, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)' }, timeout: 4000 }, (res) => {
        let data = '';
        res.on('data', chunk => {
          data += chunk;
        ...
        res.on('close', () => {
          const startMatch = data.match(/"startTimestamp":"([^"]+)"/);
          if (startMatch && startMatch[1]) {
            const startTime = new Date(startMatch[1]).getTime();
            const elapsedSec = Math.max(0, Math.floor((Date.now() - startTime) / 1000));
            const estimatedFrags = Math.floor(elapsedSec / 2.0);
            return resolve({ elapsedSec, estimatedFrags });
          }
          resolve(null);
        });
      });
...
async function runLivestream(opts, broadcast) {
  ...
  detectLiveStreamEdge(opts.url).then(info => {
    if (info && info.estimatedFrags > 0) {
      const h = Math.floor(info.elapsedSec / 3600).toString().padStart(2, '0');
      const m = Math.floor((info.elapsedSec % 3600) / 60).toString().padStart(2, '0');
      const s = Math.floor(info.elapsedSec % 60).toString().padStart(2, '0');
      broadcast({
        type: 'stdout',
        text: `[live] Detected live broadcast edge: ~${info.estimatedFrags.toLocaleString()} fragments (stream elapsed: ${h}:${m}:${s})\n`
      });
    }
  }).catch(() => {});
...
async function runYtdlp(opts, broadcast) {
  ...
  if (opts.cookiesPath && fs.existsSync(opts.cookiesPath)) args.push('--cookies', opts.cookiesPath);
  if (opts.concurrent && parseInt(opts.concurrent) > 1) {
    args.push('--concurrent-fragments', String(opts.concurrent));
  }
  if (opts.client && opts.client !== 'default') {
...
function stopProc(pid) {
  if (pid !== undefined && pid !== null && pid !== '') {
    ...
    const p = activeProcs.get(pid) || (numPid ? activeProcs.get(numPid) : null) || (typeof pid === 'number' ? activeProcs.get(String(pid)) : null);
    if (p) {
      p._isKilledByUser = true;
      if (p.opts) p.opts._isAborted = true;
      if (typeof p.kill === 'function') {
        try { p.kill('SIGKILL'); } catch (e) { }
      }
    }
    killProcess(pid);
    if (numPid && numPid !== pid) killProcess(numPid);
    activeProcs.delete(pid);
    if (numPid) activeProcs.delete(numPid);
    if (typeof pid === 'number') activeProcs.delete(String(pid));
  } else {
    for (const [procPid, proc] of activeProcs.entries()) {
      proc._isKilledByUser = true;
      if (proc.opts) proc.opts._isAborted = true;
