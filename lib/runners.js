...
async function runGalleryDl(opts, broadcast) {
  ...
  let gdlDir = path.join(__dirname, '..', 'vendor', 'gallery-dl');
  if (process.versions && process.versions.electron && __dirname.includes('app.asar')) {
    gdlDir = path.join(require('electron').app.getPath('userData'), 'vendor', 'gallery-dl');
  }
...
async function runConcatenator(opts, broadcast) {
  ...
  if (opts.forceEncode) {
    ...
    const args = ['-y', '-f', 'concat', '-safe', '0', '-i', listFile, '-c:v', 'libx264', '-preset', 'fast', '-c:a', 'aac', opts.output];
    const child = spawnRunner(ffmpeg, args, opts, broadcast);
    child.on('close', () => { try { fs.unlinkSync(listFile) } catch(e){} });
  } else {
    ...
    for (let i = 0; i < opts.files.length; i++) {
      ...
      broadcast({ type: 'stdout', text: `\n[Concatenator] Preparing part ${i+1}/${opts.files.length} (normalizing timestamps)...\n` });
