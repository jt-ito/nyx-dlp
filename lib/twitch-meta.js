      ...
      if (user) {
        ...
        if (matchedVideo) {
          return {
            ...
            displayName,
            profileImage,
            gameId: matchedVideo.game?.id,
            gameName: matchedVideo.game?.name,
...
async function resolveTwitchVodMetadata(opts = {}) {
  ...
  // If live stream (timestamp within last 24h) and no archive title, use current live title
  const isRecentStream = timestamp && (Date.now() / 1000 - timestamp) < 86400;
