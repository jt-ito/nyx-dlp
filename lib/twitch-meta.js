...
function extractTwitchInfoFromText(text) {
  ...
  // 2. Vodvod URL: https://api.vodvod.top/m3u8/[streamId]/[token]/index.m3u8
  const vodvodMatch = text.match(/vodvod\.top\/m3u8\/([0-9]+)/i);
  if (vodvodMatch) {
    result.streamId = vodvodMatch[1];
  }

  // 3. CloudFront chunk pattern: [hash]_[channel]_[streamId]_[token]
  ...
  // 4. Program Date Time in HLS manifest: #EXT-X-PROGRAM-DATE-TIME:2026-08-25T01:07:53.1Z
  const pdtMatch = text.match(/#EXT-X-PROGRAM-DATE-TIME:([^\r\n]+)/i);
  if (pdtMatch) {
    const d = new Date(pdtMatch[1].trim());
    if (!isNaN(d.getTime())) {
      result.timestamp = Math.floor(d.getTime() / 1000);
      result.programDateTime = pdtMatch[1].trim();
      result.createdAt = pdtMatch[1].trim();
    }
  }

  // 5. Twitch video URL: https://www.twitch.tv/videos/[videoId]
      ...
      if (user) {
        ...
        if (timestamp && videos.length > 0) {
          ...
          for (const v of videos) {
            ...
            // Match closest stream broadcast start within 2 hours (7200 seconds)
        ...
        // If no timestamp provided, treat as latest stream
        if (!matchedVideo && !timestamp && videos.length > 0) {
...
async function fetchDecapiInfo(channel) {
  ...
  try {
    ...
    const isDecapiValid = (res) => res.status === 200 && res.data && !res.data.includes('404') && !res.data.toLowerCase().includes('error occurred') && !res.data.toLowerCase().includes('no user found');
    const profileImage = isDecapiValid(avatarRes) ? avatarRes.data.trim() : null;
    const currentTitle = isDecapiValid(titleRes) ? titleRes.data.trim() : null;
    const currentGame = isDecapiValid(gameRes) ? gameRes.data.trim() : null;
...
/**
 * Fetch stream metadata directly from VodVod API (public, no Cloudflare)
 */
async function fetchVodvodStreamInfo(channel, streamId, timestamp) {
  if (!channel) return null;
  try {
    const cleanChan = channel.replace(/^@/, '');
    const res = await fetchUrl(`https://api.vodvod.top/channels/@${encodeURIComponent(cleanChan)}`, {
      'Accept': 'application/json, text/plain, */*',
      'Referer': 'https://vodvod.top/'
    }, 3500);
    if (res.status === 200 && res.data) {
      ...
      if (Array.isArray(list)) {
        let match = null;
        if (streamId) {
          match = list.find(s => String(s.Metadata?.StreamID) === String(streamId) || (s.Link && s.Link.includes(String(streamId))));
        }
        if (!match && timestamp) {
          let minDiff = Infinity;
          for (const s of list) {
            if (s.Metadata?.StartTime) {
              const sTime = Math.floor(new Date(s.Metadata.StartTime).getTime() / 1000);
              const diff = Math.abs(sTime - timestamp);
              if (diff < minDiff && diff < 7200) {
                minDiff = diff;
                match = s;
              }
        ...
        if (match && match.Metadata) {
          return {
            title: match.Metadata.TitleAtStart || null,
            gameName: match.Metadata.GameNameAtStart || null,
            gameId: match.Metadata.GameIDAtStart || null,
            createdAt: match.Metadata.StartTime || null,
            profileImage: match.Metadata.ProfileImageUrlAtStart?.String || null,
            streamId: match.Metadata.StreamID || streamId
          };
        }
      }
    }
...
/**
 * Scrape exact stream title and category from TwitchTracker stream page
 */
async function scrapeTwitchTrackerStream(channel, streamId) {
  ...
  try {
    ...
    return await new Promise((resolve) => {
      ...
      const targetUrl = `https://twitchtracker.com/${encodeURIComponent(channel)}/streams/${encodeURIComponent(streamId)}`;
        ...
        try {
          ...
          const res = await win.webContents.executeJavaScript(`
            (() => {
              ...
              const idxTitle = text.indexOf('STREAM TITLE');
              if (idxTitle !== -1) {
                const after = text.substring(idxTitle + 12).trim();
                const lines = after.split('\\n').map(l => l.trim()).filter(Boolean);
                if (lines.length > 0 && !lines[0].startsWith('PLAYED')) {
                  title = lines[0];
                }
              }

              const idxGame = text.indexOf('PLAYED GAMES');
              if (idxGame !== -1) {
                const after = text.substring(idxGame + 12).trim();
                const lines = after.split('\\n').map(l => l.trim()).filter(Boolean);
                if (lines.length > 0) {
                  game = lines[0];
                }
      ...
      win.webContents.on('dom-ready', tryExtract);
      const check = setInterval(tryExtract, 200);
...
async function resolveTwitchVodMetadata(opts = {}) {
  ...
  const cacheKey = (url || `${channel || ''}_${streamId || videoId || timestamp || ''}`).trim().toLowerCase();
  if (cacheKey) {
  ...
  // 3. Concurrently fetch metadata from Twitch GQL, DecAPI, TwitchTracker channel summary, VodVod API, and stream scraper
  const scrapePromise = (channel && streamId && !videoId)
    ? scrapeTwitchTrackerStream(channel, streamId)
    : Promise.resolve(null);

  const [gqlMeta, decapiMeta, channelSummary, ttStream, vodvodMeta] = await Promise.all([
    ...
    scrapePromise,
    channel ? fetchVodvodStreamInfo(channel, streamId, timestamp) : Promise.resolve(null)
  ]);
  ...
  const isRecentStream = !timestamp || (Date.now() / 1000 - timestamp) < 21600;

  // Video title matching logic
  let title = gqlMeta?.title || null;
  let matchedVideoId = gqlMeta?.videoId || videoId || null;
  let gameName = gqlMeta?.gameName || (isRecentStream ? decapiMeta?.currentGame : null);
  let gameId = gqlMeta?.gameId || null;
  let createdAt = gqlMeta?.createdAt || opts.createdAt || null;
  ...
  // If title not yet resolved and stream is recent, fallback to latest stream video title from DecAPI
  if (!title && isRecentStream && decapiMeta?.videos?.length > 0) {
    title = decapiMeta.videos[0].title;
    matchedVideoId = decapiMeta.videos[0].videoId;
  }

  // Fallback to channel's current live stream title only if the stream is currently ongoing / recent
  if (!title && isRecentStream && decapiMeta?.currentTitle) {
    title = decapiMeta.currentTitle;
  }

  // Fallback to VodVod archive metadata for historical / unlisted streams
  if (!title && vodvodMeta?.title) {
    title = vodvodMeta.title;
    if (!gameName && vodvodMeta.gameName) {
      gameName = vodvodMeta.gameName;
    }
    if (!gameId && vodvodMeta.gameId) {
      gameId = vodvodMeta.gameId;
    }
    if (!createdAt && vodvodMeta.createdAt) {
      createdAt = vodvodMeta.createdAt;
    }
    if (!profileImage && vodvodMeta.profileImage) {
      profileImage = vodvodMeta.profileImage;
    }
  }

  // Fallback to TwitchTracker stream page for historical streams without an active Twitch VOD
  if (!title && ttStream?.title) {
    title = ttStream.title;
    if (!gameName && ttStream.game) {
      gameName = ttStream.game;
    }
  }
  ...
  if (cacheKey && (result.title || result.channel)) {
    metadataMemoryCache.set(cacheKey, { data: result, timestamp: Date.now() });
  }
