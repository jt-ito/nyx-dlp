...
async function resolveTwitchVodMetadata(opts = {}) {
  ...
  // 3. Concurrently fetch metadata from Twitch GQL, DecAPI, TwitchTracker channel summary, and background stream scraper
  const scrapePromise = (channel && streamId && !videoId)
    ? scrapeTwitchTrackerStream(channel, streamId)
    : Promise.resolve(null);

  const [gqlMeta, decapiMeta, channelSummary, ttStream] = await Promise.all([
